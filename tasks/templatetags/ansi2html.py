...
def _ansi2html_get_styles(palette):
    if palette not in _ANSI2HTML_STYLES:
        ...
        for i in range(8):
            ...
            regular_style['9%s' % i] = 'color:%s' % p[i]
            regular_style['10%s' % i] = 'background-color:%s' % p[i]

            bold_style['3%s' % i] = 'color:%s' % p[i + 8]
            bold_style['4%s' % i] = 'background-color:%s' % p[i + 8]
            bold_style['9%s' % i] = 'color:%s' % p[i + 8]
            bold_style['10%s' % i] = 'background-color:%s' % p[i + 8]
...
@register.filter('ansi2html')
def _ansi2htmlfilter(content):
    return ansi2html(content,'console')
